...
__host__ void update_eta_index(float* eta,  int* eta_index_array, int step) {
    int index_eta;
    ...
    for (int i = 0; i < Nx; i++) {
        for (int j = 0; j < Ny; j++) {
            for (int k = 0; k < Nz; k++) {
                ...
                int index3D = i * Ny * Nz + j * Nz + k;  // Convert (i, j, k) to a 1D index
                if (sum_eta_pow2 < 0.8) {
                    eta_index_array[index3D] = 0;  // Store 0 if sum_eta_pow2 is less than 0.8
                }
                else {
                    eta_index_array[index3D] = max_index + 1;  // Store max_index + 1 otherwise
                }
            }
        }
    }
...
__host__ void read_eta_file_complex(float* eta, int step) {
    std::ifstream inputfile;
    inputfile.open("eta" + std::to_string(step) + ".txt");

    if (!inputfile.is_open()) {
        std::cerr << "Error: eta" << step << ".txt cannot be opened" << std::endl;
        exit(1);
    }
        ...
        while (std::getline(linestream, data, ' ')) {
            if (n >= 3 && n < Norient + 3) {
                try {
                    ...
                    std::cerr << "Error: Invalid argument encountered when parsing data: " << data << std::endl;
                    exit(1);
                } catch (const std::out_of_range& e) {
                    std::cerr << "Warning: Out of range error encountered when parsing data: " << data 
                              << ". Setting value to 0." << std::endl;
                    eta[m] = 0.0f;  // 将超出范围的小值设置为 0
                    m++;
                }
            }
            n++;
...
// __global__ void updateeta(float* eta, float* RHS, float* energy, float* temp) {
//     // get current index
...
__global__ void updateeta_withBC_path1(float* eta, float* RHS, float* energy, float* temp, int* materialsID, int* eta_index_array) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (index < d_Nx * d_Ny * d_Nz * d_Norient) {
        ...
        if(materialID_center != 0) {
            float L;
            if (eta_index_array[index1D_center] == 1) { // 如果 eta_index 是 1
                L = expf(- d_Gm1 / (d_kb * temp[0])); // 设定 L 为较大值
            } else {
                L = 0.00000001; // 其他情况 L 为 0
            }
            eta[index] = eta[index] + d_dt * RHS[index] * L;
        } else {
            eta[index] = 0.0f;
        }
    }
...
__global__ void updateeta_withBC_path1_check(float* eta, float* RHS, float* energy, float* temp, int* materialsID, int* eta_index_array, float* L_array){
    int index = blockIdx.x * blockDim.x + threadIdx.x;
...
int main() {
    ...
    int* eta_index_array;
    eta_index_array = (int*)malloc(Nx * Ny * Nz * sizeof(int));  // Allocate memory for eta_index_array
    ...
    int* d_eta_index_array;
    checkCuda(cudaMalloc(&d_eta_index_array, Nx * Ny * Nz * sizeof(int)));
    ...
    read_eta_file(eta, 0);
    //read_eta_file_complex(eta, 0);
    output_eta(eta, 0);
    //update the eta index
    //output_eta_index(eta, 0);
    update_eta_index(eta, eta_index_array, 0);
    checkCuda(cudaMemcpy(d_eta_index_array, eta_index_array, Nx * Ny * Nz * sizeof(int), cudaMemcpyHostToDevice));

    // Allocate memory for checking
    int* h_check_array = new int[Nx * Ny * Nz];
    // Copy data back from device to host
    checkCudaError(cudaMemcpy(h_check_array, d_eta_index_array, Nx * Ny * Nz * sizeof(int), cudaMemcpyDeviceToHost), "Failed to copy data back to host");
    // Check if data is the same
    bool success = true;
    for (int i = 0; i < Nx * Ny * Nz; i++) {
        if (eta_index_array[i] != h_check_array[i]) {
            std::cerr << "Mismatch at index " << i << ": host=" << eta_index_array[i] << ", device=" << h_check_array[i] << std::endl;
            success = false;
            break;
        }
    }
    if (success) {
        std::cout << "Data successfully copied to the device and verified!" << std::endl;
    } else {
        std::cerr << "Data verification failed!" << std::endl;
    }
    delete[] h_check_array;
    ...
    for (int s = 1; s <= Nstep; s++) {
        ...
        updateeta_withBC_path1 << <numblocks, blocksize >> > (d_eta, d_RHS, d_energy, d_temp, d_materialIDs,d_eta_index_array);
        cudaDeviceSynchronize();

        //checking!!!!!!!!!
        // float* h_L_array = new float[Nx * Ny * Nz];
        // float* d_L_array;
        // cudaMalloc((void**)&d_L_array, Nx * Ny * Nz * sizeof(float));
        // updateeta_withBC_path1_check << <numblocks, blocksize >> > (d_eta, d_RHS, d_energy, d_temp, d_materialIDs, d_eta_index_array, d_L_array);
        // cudaDeviceSynchronize();
        // cudaMemcpy(h_L_array, d_L_array, Nx * Ny * Nz * sizeof(float), cudaMemcpyDeviceToHost);

        //update the eta index
        checkCuda(cudaMemcpy(eta, d_eta, Nx * Ny * Nz * Norient * sizeof(float), cudaMemcpyDeviceToHost));
        update_eta_index(eta, eta_index_array, s);
        checkCuda(cudaMemcpy(d_eta_index_array, eta_index_array, Nx * Ny * Nz * sizeof(int), cudaMemcpyHostToDevice));

        // // checking!!!!!!!!! 将 eta_index_array 和 L 输出到文件
        // checkCuda(cudaMemcpy(h_check_array, d_eta_index_array, Nx * Ny * Nz * sizeof(int), cudaMemcpyDeviceToHost));
        // // 使用 step 生成文件名
        // std::string filename = "L_values_step" + std::to_string(s) + ".txt";
        // std::ofstream outputfile(filename);

        // // 输出 eta_index_array 和 L 到文件
        // for (int i = 0; i < Nx; i++) {
        //     for (int j = 0; j < Ny; j++) {
        //         for (int k = 0; k < Nz; k++) {
        //             int index3D = i * Ny * Nz + j * Nz + k;
        //             outputfile << i << " " << j << " " << k << " "
        //                     << h_check_array[index3D] << " "
        //                     << h_L_array[index3D] << std::endl;
        //         }
        //     }
        // }
        // outputfile.close();
    ...
    free(grainvol);
    free(materialIDs);
    free(eta_index_array);
    cudaFree(d_eta);
    cudaFree(d_RHS);
    cudaFree(d_materialIDs);
    cudaFree(d_eta_index_array);
